(function (factory) {
    ...
    return function koreBotChat() {
        ...
        var returnFun =(function ($,KRPerfectScrollbar) {
            ...
            function sortSpeakText(data,obj){
                let text = data;
                if(!data){
                    ...
                    switch (type) {
                        case 'quick_replies':
                            const titlesString = payload.quick_replies.map(item => item.title).join(', ');
                            text += `Options  ${titlesString}`
                            break;
